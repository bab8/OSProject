...
static uint32_t read_raw_data(uint32_t cluster_index, char* buffer, uint32_t size){
    ...
    //check for valid cluster index, if index is invalid return max val indicating read operation fail
    if(index < 2){
        return 0xffffffff;
    }

    //copy data from each cluster to buffer and use amount of data read as loop counter
    while(read_size < size){
        ...
        //being greater than 0xfff7 means this is the last cluster to read
        if(index >= 0xfff7){
