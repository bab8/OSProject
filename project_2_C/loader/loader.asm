...
start:
    ...
    mov ax,0x2000; used to find memory location
    mov es,ax
...
GetMemInfoStart:;use to get info on memory address block to see what memory is avaible to be used
    ...
    mov dword[es:0],0;struture, es used to access memory
    mov edi,8; save the memory address
    xor ebx,ebx
    int 0x15
    jc NotSupport; service 0xe280 not available

GetMemInfo:
    cmp dword[es:di+16],1; check memory type to see if it is free mem
    jne Cont
    cmp dword[es:di+4],0; check base address to see if it is within range (4gb)
    jne Cont
    mov eax,[es:di];move lower part of address
    cmp eax,0x30000000; where memory resides in ram, check against lower part of address
    ja Cont
    cmp dword[es:di+12],0; if higher part of address is not 0 then it is larger than the 4gb region
    jne Find
    add eax,[es:di+8]; load next part of address tehn cmp to size of img
    cmp eax,0x30000000 + 100*1024*1024; memory size for file system
    jb Cont
...
BigRealMode:
    sti
    mov cx,203*16*63/100;chs value this value will grab total sectors, cx will acta as a counter
    xor ebx,ebx
    mov edi,0x30000000; holds high mem address we load into
    xor ax,ax
    mov fs,ax; clear fs register to avoid unexpected issues when loading
...
ReadSectors:
    ...
    mov dword[si+8],ebx; ebx will be passed in to determien amount of sectors read
    mov dword[si+0xc],0
    mov dl,[DriveId]
    mov ah,0x42
    int 0x13
...
LMEntry:
    ...
    mov rdi,0x100000; destination address stored in rdi
    mov rsi,CModule; source address stored in rsi, current location of kernel is CModule
    mov rcx,512*15/8; rcx acts as counter
    rep movsq; repeat mov quad word rcx times

    mov rax, 0xffff800000100000; virtual address of kernel
