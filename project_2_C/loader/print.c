#include "stdint.h"
#include "stdarg.h"
#include "print.h"
#include "lib.h"

//struct to hold screen print position
static struct ScreenBuffer screen_buffer = {(char*)(0xb8000), 0, 0};

static int udecimal_to_string(char* buffer, int position, uint64_t digits){
    char digits_map[10] = "0123456789";
    char digits_buffer[25];
    int size = 0;

    do{
        digits_buffer[size++] = digits_map[digits % 10];
        digits /= 10;
    } while (digits != 0);

    for(int i = size - 1; i >= 0; i--){
        buffer[position++] = digits_buffer[i];
    }

    return size;
}

static int decimal_to_string(char* buffer, int position, int64_t digits){
    int size = 0;

    //check for negative value, if so add - and convert to positive
    if(digits < 0){
        digits = -digits;
        buffer[position++] = '-';
        size = 1;
    }

    //use unsinged function to convert the now 'posiitive' int
    size += udecimal_to_string(buffer, position, (uint64_t)digits);
    return size;
}

static int hex_to_string(char* buffer, int position, uint64_t digits){
    char digits_map[16] = "0123456789ABCDEF";
    char digits_buffer[25];
    int size = 0;

    do{
        digits_buffer[size++] = digits_map[digits % 16];
        digits /= 16;
    } while (digits != 0);

    //add hexadecimal prefix
    buffer[position++] = '0';
    buffer[position++] = 'x';
    //digits copied in backwards so have to start with high index and count to zero to preserve proper order
    for(int i = size - 1; i >= 0; i--){
        buffer[position++] = digits_buffer[i];
    }

    //buffer[position++] = 'H';
    //add 2 to size to account for prefix added
    return size + 2;

}

static int read_string(char* buffer, int position, const char* string){
    int index = 0;

    for(index = 0; string[index] != '\0'; index++){
        buffer[position++] = string[index];
    }

    //number of char put into buffer
    return index;
}

void write_screen(const char* buffer,int size, char color){
    struct ScreenBuffer *sb = &screen_buffer;
    int column = sb->column;
    int row = sb->row;

    for(int i = 0; i < size; i++){
        //scrolling screen
        if(row >= 25){
            //copy the line
            memcpy(sb->buffer,sb->buffer+LINE_SIZE,LINE_SIZE*24);
            //empty the last line
            memset(sb->buffer+LINE_SIZE*24,0,LINE_SIZE);
            row--;
        }
        //move row for newline char
        if(buffer[i] == '\n'){
            column = 0;
            row++;
        }
        else if(buffer[i] == '\b'){
            //if at beginning of screen do nothing
            if(column == 0 && row == 0){
                continue;
            }
            //if at end of column go back a line
            if(column == 0){
                row--;
                column = 80;
            }

            column -= 1;
            sb->buffer[column*2+row*LINE_SIZE] = 0;
            sb->buffer[column*2+row*LINE_SIZE+1] = 0;
        }
        //print char to screen and move position
        else{
            sb->buffer[column*2+row*LINE_SIZE] = buffer[i];
            sb->buffer[column*2+row*LINE_SIZE+1] = color;

            column++;

            if(column >= 80){
                column = 0;
                row++;
            }
        }
    }

    sb->column = column;
    sb->row = row;
}

int printk(const char *format, ...){
    char buffer[1024];
    int buffer_size = 0;
    int64_t integer = 0;
    char* string = 0;
    va_list args;

    //init args list
    va_start(args,format);

    //parse format string
    for(int i = 0; format[i] != '\0'; i++){
        //regular char just add to buffer and increase size
        if(format[i] != '%'){
            buffer[buffer_size++] = format[i];
        }
        else{
            //this is to support format strings like %s, %d, so that print stmts can function like printf in C
            switch (format[++i])
            {
            //hexadecimal
            case  'x':
                integer = va_arg(args, int64_t);
                buffer_size += hex_to_string(buffer, buffer_size, (uint64_t)integer);
                break;
            //unsigned int
            case 'u':
                integer = va_arg(args, int64_t);
                buffer_size += udecimal_to_string(buffer, buffer_size, (uint64_t)integer);
                break;
            //signed int
            case 'd':
                integer = va_arg(args, int64_t);
                buffer_size += decimal_to_string(buffer, buffer_size, integer);
                break;
            //string
            case 's':
                //input won't be a number so va_args should expect a character ptr instead for a string
                string = va_arg(args, char*);
                buffer_size += read_string(buffer, buffer_size, string);
                break;
                //unsupported format string add % to buffer then deincrement to reporcess current char
            default:
                buffer[buffer_size++] = '%';
                i--;
            }
        }
    }

    //0xf is our text color(white)
    write_screen(buffer,buffer_size, 0xf);
    va_end(args);

    return buffer_size;
}