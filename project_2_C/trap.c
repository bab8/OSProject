...
static void init_idt_entry(struct IdtEntry *entry, uint64_t addr, uint8_t attribute){
    ...
    entry->selector = 8;//kernel code segment selector
...
void init_idt(void){
    //init idt entries
    init_idt_entry(&vectors[0],(uint64_t)vector0,0x8e); //6th byte is for attributes p-dpl-type in this case 1-00-0110, 0110 means interrupt gate descriptor, 0x8e = 1000110
